...
#    # 行動計の範囲を変える時はここを変える
   try:   
      before_active_count_start_row = think_range.full_start_active_counter_row(before_data)
   except:
      before_active_count_start_row = 0
   try:
      before_active_count_finish_row = think_range.full_finish_active_counter_row(before_data)
   except:
...
with open('answer2/no4_answer.csv', 'w', newline='') as f:
   writer = csv.writer(f)
   for row in answer:
      writer.writerow(row)
...
with open('answer2/no5_answer.csv', 'w', newline='') as f:
   writer = csv.writer(f)
   for row in answer:
      writer.writerow(row)